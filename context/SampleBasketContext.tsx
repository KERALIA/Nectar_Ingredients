...
export interface BasketItem {
  ...
  sku: string
  category: string
  quantity: number   // per-item quantity in kg, default 1
}
...
interface SampleBasketContextType {
  ...
  toggleBasket: (item: Omit<BasketItem, 'quantity'>) => void
...
export function SampleBasketProvider({ children }: { children: React.ReactNode }) {
    ...
    try {
      ...
      if (stored) {
        ...
        if (Array.isArray(parsed)) {
          // Back-compat: items stored before quantity was added default to 1
          setBasket(parsed.map((item: Omit<BasketItem, 'quantity'> & { quantity?: number }) => ({
            quantity: 1,
            ...item,
          })))
        }
  ...
  const toggleBasket = useCallback((item: Omit<BasketItem, 'quantity'>) => {
    setBasket((prev) => {
  ...
  const setItemQuantity = useCallback((id: string, qty: number) => {
    setBasket((prev) =>
      prev.map((b) => (b.id === id ? { ...b, quantity: Math.max(1, qty) } : b))
    )
  }, [])
