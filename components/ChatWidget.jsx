...
export default function ChatWidget() {
  ...
  return (
    <div ref={widgetRef} className="fixed bottom-5 right-5 z-[1000] font-body flex flex-col items-end">
      {isOpen && (
        <div className="w-[calc(100vw-2rem)] sm:w-[380px] h-[500px] max-h-[75vh] bg-white dark:bg-[#1A1A1D] border border-neutral-200 dark:border-neutral-800 rounded-2xl shadow-2xl flex flex-col mb-3 overflow-hidden animate-scale-up">
          {/* Chatbot Header */}
          <div className="bg-[#BC4B20] text-white px-4 py-3.5 flex justify-between items-center shadow-md">
            <div className="flex items-center gap-2">
              <span className="text-xl">🤖</span>
              <div>
                <span className="font-bold text-base block leading-none">Nectar Ingredients</span>
                <span className="text-xs text-white/80">AI Assistant · Online 🟢</span>
              </div>
            </div>
            <button
              onClick={() => setIsOpen(false)}
              aria-label="Close chat"
              className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-white text-xl cursor-pointer transition-colors"
            >
              ×
            </button>
          </div>
          ...
          <div
            ...
            className="flex-1 overflow-y-auto p-4 flex flex-col gap-3 bg-neutral-50 dark:bg-[#141416] overscroll-contain lenis-prevent"
            style={{ overscrollBehavior: "contain", touchAction: "pan-y" }}
          >
            {messages.map((m, i) => (
              <div
                key={i}
                className={`max-w-[88%] px-4 py-3 rounded-2xl text-sm sm:text-base leading-relaxed transition-all shadow-sm ${
                  m.role === "user"
                    ? "self-end bg-[#BC4B20] text-white rounded-br-none font-medium"
                    : "self-start bg-white dark:bg-[#202024] text-neutral-900 dark:text-neutral-100 rounded-bl-none border border-neutral-200/80 dark:border-neutral-700/60"
                }`}
              >
          ...
          {/* Chatbot Input form */}
          <form onSubmit={sendMessage} className="flex border-t border-neutral-200 dark:border-neutral-800 p-3 gap-2 bg-white dark:bg-[#1A1A1D]">
            <input
              ref={inputRef}
              value={input}
              ...
              disabled={isSending}
              className="flex-1 bg-neutral-100 dark:bg-[#242428] border border-neutral-300 dark:border-neutral-700 rounded-full px-4 py-2.5 text-sm sm:text-base text-neutral-900 dark:text-white placeholder:text-neutral-500 outline-none focus:ring-2 focus:ring-[#BC4B20]/50 transition-all"
            />
            <button
              type="submit"
              disabled={isSending || !input.trim()}
              className={`bg-[#BC4B20] text-white border-none rounded-full px-4 py-2.5 text-xs sm:text-sm font-bold uppercase tracking-wider active:scale-[0.97] transition-all cursor-pointer shadow-sm ${
      ...
      <button
        ...
        className="w-14 h-14 rounded-full bg-[#BC4B20] text-white border-none flex items-center justify-center text-2xl shadow-2xl hover:scale-105 active:scale-95 transition-all cursor-pointer touch-manipulation"
        style={{
